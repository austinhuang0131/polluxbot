...
if(msg.content.split(/ +/).slice(1)[0]=="frame"){
 let ag= msg.content.split(/ +/).slice(1)[1];
  ...
  function switchon(){
    gear.userDB.set(msg.author.id,{$set:{'switches.profileFrame':true}}).then(x=>msg.react(':switchon:343511231434588161'));
  }
  function switchoff(){
    gear.userDB.set(msg.author.id,{$set:{'switches.profileFrame':false}}).then(x=>msg.react(':switchoff:343511248085843968'));
  }
...
const Server = msg.guild
       const Target = await gear.getTarget(msg);
        let TARGET_DB   = await userDB.findOne({id:Target.id}) ;
...
if (TARGET_DB.modules.medals.length < 9) {  
  ...
  for(i=0;i<lent;i++){
    await gear.userDB.set(Target.id,{$push:{'modules.medals':0}});
  }
...
let globalrank;
    ...
    globalrank  = userDB.find({"modules.exp": { $gt : TARGET_DB.modules.exp},blacklisted:{$exists:false}}).countDocuments();
msg.channel.stopTyping();
    ...
    try{
     ...
     propic = (Target.user||Target).displayAvatarURL({format:"png"})
    }catch(e){
...
let l_exp,
  ...
  async function XChart(size, pcent, colorX,pic,lvthis) {
    ...
    if(pic){
      context.clip();
      let a = await gear.getCanvas(pic);
    ...
    let tg = await gear.tag(context,lvthis,"900 56px 'Whitney HTF'","#363636");
...
c.globalCompositeOperation='destination-atop';
  ...
  let mainframe =  gear.getCanvas(paths.BUILD+"/profile/"+(Target.bot?PFLD?"mainframe_botpart":"mainframe_bot":"mainframe")+".png"),
      _bg =  gear.getCanvas(paths.BUILD+"/backdrops/"+backgroundId+".png"),
      _flairTop =  gear.getCanvas(paths.BUILD+"/flairs/top/"+flair+".png");
  ...
  for(i=0;i<medals.length;i++){
    ...
    valid_medals++;  
      ...
      let imge = await gear.getCanvas(paths.MEDALS+md+".png");
      //console.log(imge)
      //console.log(paths.MEDALS+md+".png")
      valid_images.push(imge);
    }
      
    //await medals_images.push(await gear.getCanvas(paths.MEDALS+md+".png"));
  }    
  }
     

  let pre_a = Server.iconURL({format:'png'})||"https://orig00.deviantart.net/b86e/f/2016/343/a/e/cosmog_discord_icon_by_zelakantal-dar345n.png"
  ...
  let hex_pic =   await Hex(220,propic);
   ...
   let label = await gear.tag(ctx,nickname,"900 44px 'Whitney HTF'","#223");
   let tagliney = await gear.tag(ctx,tagline,"100 16px 'Whitney HTF'","#445");

    // REPUTATION ============================================================>
   rep = rep > 999 ? 999 : rep;
   let _rep = await gear.tag(ctx,rep,"40px 'Visitor TT1 BRK'","#fff");
   let rep_displace= _rep.width/2
   
    // LOVEPOINTS ============================================================>
   let lovpoints = TARGET_DB.modules.lovepoints > 999 ? 999 : TARGET_DB.modules.lovepoints;
   let _love = await gear.tag(ctx,lovpoints||"0","40px 'Visitor TT1 BRK'","#fff");
  ...
  if(marriagestatus && marriagestatus.length > 0){
    try{
      ...
      let marriTo = "Married to:"//mm('marriage.marriedTo',P)
      let partner = await msg.botUser.users.fetch(marriage_partner.id);
      let WIFE = partner.username
      let WIFEavi = await gear.getCanvas(partner.displayAvatarURL({format:'png',size:64}) );
      
      let marRing = await gear.getCanvas('http://pollux.fun/build/items/'+(marriage_partner.ring)+'.png')
          ,married_tex = await gear.tag(cxx,marriTo,"500 18px 'Whitney HTF',Sans","#3a3a4d")
          ,married_tag = await gear.tag(cxx,WIFE,"900 36px 'Whitney HTF',Sans","#3a3a4d");      
      ...
      let eversince = 
        ...
        let married_eversince = await gear.tag(cxx," "+eversince+unit+"","600 20px 'Whitney HTF',Sans","#3a3a4d");
      ...
      try{
       cxx.drawImage(WIFEavi,5,5,50,50);
        ...
        let wifeavi2 =await gear.getCanvas(msg.author.displayAvatarURL({format:"png"}));
  ...
  ctx.drawImage(_rep.item,_REP.x-rep_displace,_REP.y)    
    ...
    if(TARGET_DB.married&&TARGET_DB.married.length>0){
      ctx.globalAlpha  = .5
      let hart = await gear.getCanvas(paths.BUILD+"hearticon.png");
  ...
  if(sticker){
    let sticky = await gear.getCanvas(paths.BUILD+"/stickers/"+sticker+".png");
    ctx.drawImage(sticky,_STICKER.x-10,_STICKER.y-25,170, 170)
  }else{
    let polluxi = await gear.getCanvas(paths.BUILD+"/polluxi.png");
    ctx.drawImage(polluxi,_STICKER.x,_STICKER.y-20)
  } 

   let personal = await gear.block(ctx,persotex,"18px 'Whitney HTF', sans-serif","#000",275,70)
   ctx.drawImage(personal.item,290,490)
  
  let rubicount = await gear.tag(ctx,rubines,
                                 "900 30px 'Whitney HTF',Sans","#2b2b2b")
  if(!Target.bot)ctx.drawImage(rubicount.item,_RUBINES.x-rubicount.width,_RUBINES.y);
  
    let REP = await gear.tag(ctx,"REP",
                               "900 30px 'Whitney HTF',Sans","#ffffff")
    ctx.globalAlpha = .6;
  ctx.drawImage(REP.item,52-REP.width/2,25)  
    ctx.globalAlpha = 1;
    let grank = await gear.tag(ctx,"#"+gear.miliarize((await globalrank)),
    ...
    while(ind<8){
      let col = 0
        while(col<3){
          ...
          if(medals[ind]){
            let medalie = await gear.getCanvas(paths.MEDALS+medals[ind]+".png");
  ...
  //ctx.drawImage(medals_images[1],_MEDALS.x+100,_MEDALS.y)
    ...
    if(TARGET_DB.switches && (TARGET_DB.switches.profiled || TARGET_DB.switches.profileFrame)){
      ...
      if(tier){
        let tierframe = await gear.getCanvas(paths.BUILD+"profile/donortiers/neo"+tier+".png");
    ...
    if(TARGET_DB.switches && !TARGET_DB.switches.hideProle){
      ...
      console.log(2)
      if(bottomTag){
        let tierframe = await gear.getCanvas(paths.BUILD+"profile/bottomtags/"+bottomTag+".png");
        ctx.drawImage(tierframe,268,559);
      }
        console.log(3)
      if(bottomTag=="translator" && TARGET_DB.switches.translator){        
        let flag = await gear.getCanvas(paths.BUILD+"flags/"+TARGET_DB.switches.translator+".png");
    ...
    */
    ...
    if(TARGET_DB.blacklisted && TARGET_DB.blacklisted !=""){
       ...
       let bliste = await gear.getCanvas(paths.BUILD+"bliste.png");
