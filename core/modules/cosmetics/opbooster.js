...
message.react("⌛")
  ...
  const stickerpak = message.args[1] || message.args[0]
...
`+"------------------------------------------------\n"+
  ...
  let bosterBack = await gear.getCanvas(paths.BUILD+"boosters/"+_stickerA.series_id+".png");
...
let  MAX = 40
  ...
  for (let frame = 0; frame < MAX; frame++) {
       canvas = new Canvas.createCanvas(350, 250);
