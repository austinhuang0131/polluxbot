...
delete require.cache[require.resolve("../../archetypes/neoLootbox.js")];
const LOOT = require("../../archetypes/neoLootbox.js");  
  
  let USERDATA = msg.author.dDATA || await gear.userDB.findOne({id:msg.author.id});
  ...
  let rerollCost = ((rollNo||0)+1)*(factors+1)*175;
...
P.s = 30;
embed.setFooter(mm("CMD.timesOutIn",P),message.author.displayAvatarURL({format:'png'})|| message.author.displayAvatarURL);
...
`);
  ...
  const responses = await message.channel.awaitMessages(msg2 =>
    ...
    if (action === "keep"){
        await keeppit(LootMeta,boxaher,msg,USERDATA);
    }
    else if(action==="reroll"&&rerolls>0){
      gear.userDB.set(message.author.id,{$inc:{'modules.rubines':-rerollCost}});
      gear.audit(message.author.id,rerollCost,"lootbox_reroll","RBN");
...
async function composeItem(currentItem,base){
  ...
  switch(currentItem.type){
    ...
    case "STICKER":
      ...
      break;
    case "BOOSTER":
      ITEM = await renderBooster(currentItem);
      break;
...
async function renderBooster(stk) {
    ...
    //itemCanvas.displayName = '"['+stk.SSS.split(' ').slice(0,2).join(' ').toUpperCase()+']'+stk.name+'"' 
    itemCanvas.canvas = new Canvas.createCanvas(285,350);
    itemCanvas.displayType = "BOOSTERPAK"
    itemCanvas.event = false;
...
async function keeppit(LOOTS,boxaher,message,UDATA) {
  for (let i = 0; i < 3; i++) {
