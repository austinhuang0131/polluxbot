...
${gear.emoji('time')   } ${gear.emoji('offline')} **${v.last}** ${ moment.utc(userDaily).calendar()} **UTC**
${gear.emoji('future') } ${dailyAvailable?gear.emoji('online'):gear.emoji('dnd')} **${v.next}** ${ moment.utc(userDaily).add(20,'hours').calendar() } **UTC** 
${gear.emoji('expired')} ${streakGoes?gear.emoji('online'):gear.emoji('dnd')} **${v.expirestr}** ${streakGoes ? moment.utc(userDaily+STREAK_EXPIRE).calendar() +" **UTC** ": "---" }
${gear.emoji('expense')} ${gear.emoji('offline')} **${v.streakcurr}** \`${hardStreak }x\`(Hard) | \`${hardStreak%10 }x\`(Soft)
  ...
  if(!dailyAvailable){
    ...
    embe2.setColor('#e35555');
    embe2.setDescription(`
${gear.emoji('time')   } **${v.last}** ${ moment.utc(userDaily).calendar()}
${gear.emoji('expired')} **${v.expirestr}** ${moment.utc(userDaily+STREAK_EXPIRE).calendar() } **UTC**
    `);
