...
const init = async function (message) {
  ...
  let dbminiarray
      ...
      dbminiarray = await gear.userDB.find({id:{$in:userArray,$not:{$in:['271394014358405121','88120564400553984']}},blacklisted:{$exists:false}}).sort({'modules.rubines': -1,}).limit(10);
    }else{
      dbminiarray = await gear.userDB.find({id:{$not:{$in:['271394014358405121','88120564400553984']}},blacklisted:{$exists:false}}).sort({'modules.rubines': -1}).limit(10);
